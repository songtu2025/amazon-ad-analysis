...
REQUIRED_MARKERS = {
    ...
    "binding_count_from_rules": "productAdBindings.filter((binding) => binding.product_id === record.id).length",
    "attributed_status_copy": "已归因",
    "pending_status_copy": "待归因",
    "manage_attribution_button": "管理归因",
    "select_product_for_attribution": "setAttributionProductId(record.id)",
    "open_attribution_tab": 'setActiveTab("attribution")',
}
...
def main() -> None:
    ...
    if missing:
        fail("产品中心归因状态入口标记缺失：" + ", ".join(missing))
        ...
        json.dumps(
            {
                "status": "success",
                "checked": [
                    "product_center_attribution_status_column",
                    "binding_count_by_product",
                    "manage_attribution_navigation",
